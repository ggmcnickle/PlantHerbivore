...
plant.game<-function(u) {
	...
	#NB: u[1] is shoot, u[2] is root
		...
		Ps <- Hs - csc*u[1] - crc*u[2]  - alph*u[1]*(1-exp(-a*u[3])) #Net profit C
		dPs.dr <- (-crc)		#derivative of Ps by r
		dPs.ds <- Rc*exp(-u[1]) - csc - alph*(1-exp(-a*u[3]))		#derivative of Ps by s	

		Hr <- Rn*(1-exp(-u[2]))		#harvest of N
		Pr <- Hr - csn*u[1] - crn*u[2] - beta*u[1]*(1-exp(-a*u[3]))	#Net profit N
		dPr.dr <- Rn*exp(-u[2])-crn #derivative of Pr by r
		dPr.ds <- (-csn - beta*(1-exp(-a*u[3])))		#derivative of Pr by s

	#Herbivore investment		
		dPh.duh	<- a*u[1]*exp(-a*u[3]) - ch
...
#Loop
	while (Rn<Rn.max) {
		...
		Ps <- Hs - csc*s.temp - crc*r.temp - s.temp*(1-exp(-a*ESS[3])) 	#Net profit C
		Hr <- Rn*(1-exp(-r.temp))		#harvest of N
		Pr <- Hr - csn*s.temp - crn*r.temp - s.temp*(1-exp(-a*ESS[3]))	#Net profit N	
...
a<-ggplot(out, aes(N, lnRR.yield)) + geom_line() + ylim(-.1, 0.01)
b<-ggplot(out, aes(N, lnRR.shoot)) + geom_line() + ylim(-.1, 0.01)
c<-ggplot(out, aes(N, lnRR.root)) + geom_line()+ ylim(-.01, 0.01)
d<-ggplot(out, aes(N, damage)) + geom_line() + ylim(0,0.5)
